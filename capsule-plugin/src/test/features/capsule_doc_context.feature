...
Feature: Capsule documentary corpus injection (CAP-DOCCONTEXT)
  ...
  I want to inject the documentary corpus (referential training manuals) into the Docs channel
  ...
  Scenario: The docsContent CLI param still works when no globs are configured (retrocompat)
    ...
    When I collect the augmented context with doc content "Referential grading manual via CLI"
    Then the doc context build succeeds
    And the doc context artefact contains "CONTEXTE_DOCS"
    And the doc context artefact contains "Referential grading manual via CLI"

  Scenario: The docsGlobs CLI param resolves files and feeds the Docs channel
    Given a Capsule doc context Gradle project with the capsule plugin applied
    And EAGER governance files are present for doc context
    And documentary corpus files are present under "docs/referential"
    When I collect the augmented context with doc globs "docs/referential/**/*.adoc"
    Then the doc context build succeeds
    And the doc context artefact contains "CONTEXTE_DOCS"
    And the doc context artefact contains "Referential grading rules for content"
  ...
  Scenario: The docsGlobs CLI param overrides the docsContent string
    ...
    And EAGER governance files are present for doc context
    And documentary corpus files are present under "docs/referential"
    When I collect the augmented context with doc content "legacy string" and doc globs "docs/referential/**/*.adoc"
    Then the doc context build succeeds
    And the doc context artefact contains "Referential grading rules for content"
